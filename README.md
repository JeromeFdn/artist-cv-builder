...
range of artistic fields and multidisciplinary practices.
...
tool, and no data is sent to GitHub or any other server.

If a user clears their browser data or uses a private/incognito
window, their saved progress will not persist between sessions.
...
4. When ready, click "Print / Save PDF" and follow the
   on-screen instructions to save a clean PDF.
...
TECHNICAL NOTES
  ...
  - Web fonts loaded from Google Fonts (Barlow, Barlow
    Condensed, DM Serif Display).
  - User progress is saved to browser localStorage under the
    key "jerome_artist_cv_v2".
  - Print output uses CSS @page margin boxes for page numbers.
